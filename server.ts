...
// Explicit API catch-all handler: Ensures any /api/* route returns JSON instead of falling through to HTML index.html
app.all('/api/*', (req, res) => {
  res.status(404).json({
    success: false,
    error: `API route ${req.method} ${req.originalUrl} was not found on this server.`
  });
});
