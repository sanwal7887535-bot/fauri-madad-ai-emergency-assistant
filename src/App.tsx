...
import { generateClientSideGuidance } from './lib/aiClient';
...
export default function App() {
  ...
  const handleSubmitQuery = async (queryText: string) => {
    ...
    try {
      let data: EmergencyGuidance | null = null;
      ...
      try {
        ...
        serverError = fetchErr.message || 'Network request to server failed.';
      }
        ...
        if (contentType.includes('application/json')) {
          try {
            ...
            if (result.success && result.data) {
              ...
              serverError = result.error || 'Server error generating emergency response.';
            }
          } catch (_) {
            serverError = 'Server returned invalid JSON.';
          }
        } else {
          // Response is non-JSON HTML (e.g. static host or 404/500 page)
          const textResponse = await res.text();
          console.warn('Server returned non-JSON response:', textResponse.slice(0, 100));
          serverError = 'Server returned HTML page instead of JSON.';
        }
      }

      // Fallback: If server did not return valid guidance data, and user provided an API key
      if (!data && settings.userApiKey && settings.userApiKey.trim()) {
        console.log('Server API unavailable/non-JSON. Falling back to direct client-side Gemini call...');
        try {
          data = await generateClientSideGuidance(queryText, settings.language, settings.userApiKey.trim());
        } catch (clientErr: any) {
          throw new Error(`Personal API Key Error: ${clientErr.message || 'Failed client-side AI request'}`);
        }
      }
      ...
      if (data) {
        ...
        throw new Error(
          serverError || 'Unable to connect to AI server. Please enter a free Gemini API key in Settings or call 1122.'
        );
      }
    } catch (err: any) {
